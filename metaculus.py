...
from models import ForecastMetaculusData, ForecastType, MetaculusCommunityFullPrediction, MetaculusCommunityPrediction
import httpx
...
class MetaculusForecaster(BaseForecaster):
    ...
    def format_as_html(self, data: ForecastMetaculusData) -> str:
        return f'<b>{data.question_title}</b> (ID: {data.id_on_platform})\n'\
    ...
    def search(self, s: str, display_popular: bool = True, limit: int = 10, offset: int = 0) -> List[ForecastMetaculusData]:
        params = {
            'order_by': '-activity',
            'limit': limit,
            'offset': offset
